...
import { ArrowLeft, Bot, MessageSquare, Send, Trash2, User } from "lucide-react"
import { dashboardChatApi, ApiException } from "@/lib/api"
...
function Bubble({ message }: BubbleProps) {
  ...
  return (
    <div className={`flex gap-2.5 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      {/* Avatar */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isUser ? "bg-primary" : "bg-muted-foreground/20"}`}>
        {isUser
      ...
      <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
        ...
        {message.citations && message.citations.length > 0 && (
          <div className="mt-2 pt-2 border-t border-muted-foreground/20 space-y-1">
            <p className="text-[10px] font-medium opacity-70 mb-0.5">Sources</p>
...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res   = await dashboardChatApi.send(token, agentId, { message: text })
            ...
            {messages.length === 0 && !responding && (
              <div className="flex flex-col items-center justify-center h-full min-h-[40vh] gap-3 text-center">
                ...
                <p className="text-xs text-muted-foreground max-w-xs">
                  Type a message below to chat with your agent. Shift+Enter for a new line.
                </p>
              </div>
            )}
